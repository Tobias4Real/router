...
impl NodeTree {
	...
	pub fn build(graph_nodes: &Vec<Node>) -> Self {
		...
		println!(
			...
			format!(
				"(d: {}, ec: {}, ml: {}, s: {})",
				max_depth, element_count, max_leaf_elements, subdivisions
			)
			.magenta()
		);
		tree
...
mod tests {
	use super::NodeTree;
	use crate::{coords::Coords, node::Node};
	...
	fn position() {
		...
		assert_eq!(
			NodeTree::relative_position(node.coords, Coords::deg(48.81392, 11.34318)),
			0
		);
	}
}
