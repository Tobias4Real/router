...
fn main() {
    println!("{}", "Router. The ultimate route finder. (C) 2022 Thorben Bernhardt & Tobias Schaberl.".cyan());
    ...
    if let Some(file) = args.graph_file {
        ...
        println!("Loading the graph took {}{}.", now.elapsed().as_millis(), "ms".green());
        ...
        if let Some(source) = args.source_node {
            ...
            println!("One-to-all dijkstra took {}{}", now.elapsed().as_millis(), "ms".green());
            ...
            let target = match args.target_node {
                ...
                None => {
                    ...
                    line[0..line.len()-1].parse::<usize>().expect("Target node id must be a positive number.")
                }
            };
